...
class XtreamSession implements ContentSource {
  ...
  List<dynamic> _requireList(dynamic data) {
    ...
    if (list == null) {
      throw XtreamSessionException(
        'The server did not return valid data. '
        'It may have reached its connection limit or temporarily '
        'blocked access. Please try again in a few minutes.',
      );
    }
    return list;
  }
  ...
  Future<VodDetail> getVodInfo(String vodId) async {
    ...
    if (data is! Map) {
      throw XtreamSessionException('Movie details unavailable.');
    }
  ...
  Future<SeriesDetail> getSeriesInfo(String seriesId) async {
    ...
    if (data is! Map) {
      throw XtreamSessionException('Series details unavailable.');
    }
