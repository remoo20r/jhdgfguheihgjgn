...
class _CategorySidebarState extends State<CategorySidebar> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showContinue) {
      addRow(kContinueCategoryId, 'CONTINUE WATCHING', Icons.live_tv, widget.continueCount);
    }
    addRow(kFavoritesCategoryId, 'FAVORITES', Icons.favorite, widget.favoritesCount);
    if (widget.showAll) {
      addRow(kAllCategoryId, 'ALL', Icons.grid_view, widget.allCount);
    }
    if (widget.showRecent) {
      addRow(kRecentCategoryId, 'RECENTLY ADDED', Icons.fiber_new, widget.recentCount);
    }
