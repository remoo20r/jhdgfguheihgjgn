...
void main() {
  ...
  testWidgets('home: Back asks to exit; OK on the focused "Cancel" stays',
      (tester) async {
    await tester.pumpWidget(ProviderScope(child: _shell(const HomeScreen())));
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    expect(find.text('Exit BellaTV?'), findsOneWidget);

    // "Cancel" starts focused (D-pad dialogs): OK dismisses, app stays.
    await _pressOk(tester);
    expect(find.text('Exit BellaTV?'), findsNothing);
  ...
  testWidgets('settings: arrows reach the aspect chips, OK applies one',
      (tester) async {
    await tester.pumpWidget(ProviderScope(child: _shell(const SettingsScreen())));
    await tester.pumpAndSettle();

    // Arrival: "Add playlist" is the autofocused starting point (no
